...
\begin{frame}
    ...
    \begin{definition}[{{\cite[Def. III.1]{plesken2014}}}]
...
\begin{frame}
    \begin{definition}[{{\label{def:vol}\cite[$§3$, Thm. $7$, with remark after]{forster2012analysis3}}}]
...
\begin{frame}
    \begin{algorithm}[H]
        ...
        $fundDom \leftarrow$ triangular complex given by intersection of $halfspaces$\;
...
\section{Outlook}
\begin{frame}
    \begin{example}{Current state}
        Previously presented algorithm is already implemented as part of my masters thesis.
    \end{example}\pause
    \begin{block}{Next steps}
        \begin{itemize}
            \item Implement above in SimplicialSurfaces Package
            \item Improve algorithm by automatically running until expected volume is reached
            \item Check if $h$-vector conversion can be done more efficiently
            \item Consider numerical effects
        \end{itemize}
    \end{block}
